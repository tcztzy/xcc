...
class AotTypeBinderTests(unittest.TestCase):
    ...
    def test_binds_float_annotations(self) -> None:
        source = "def f(value: float | None) -> float:\n    return 0.0 if value is None else value\n"
        module = parse_source(source, filename="float_annotation.py")
        analysis = bind_types(check_subset(module), module)
        self.assertEqual(analysis.functions["f"].parameters, (("value", "float | None"),))
        self.assertEqual(analysis.functions["f"].return_type.name, "float")
