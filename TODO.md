...
- Next target: reconcile `tests/external/clang/manifest.json` message matchers with the newer parser diagnostics (e.g., declaration-type missing-token and unknown-type-name wording) so `tox -e py311` stops failing on stale substring expectations.
