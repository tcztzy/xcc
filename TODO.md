...
- Next target: extend parser regression coverage for the remaining compound-assignment punctuator diagnostics (`-=`, `*=`, `/=`, `%=` , `&=`, `|=`, `^=`, `>>=`) to ensure every newly mapped token is explicitly locked down.
