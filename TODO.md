...
- Next target: add sibling curated clang-manifest fixtures for these non-typedef function-pointer parameter-list diagnostics in block-scope and struct/union-member declaration contexts so external conformance coverage mirrors the new local parser regressions.
