...
def parse_expression(parser: object) -> Expr:
    expr = parser._parse_assignment()  # type: ignore
    while parser._check_punct(","):  # type: ignore
        parser._advance()  # type: ignore
        right = parser._parse_assignment()  # type: ignore
...
def parse_assignment(parser: object) -> Expr:
    ...
    if parser._is_assignment_operator():  # type: ignore
        op = str(parser._advance().lexeme)  # type: ignore
...
def parse_conditional(parser: object) -> Expr:
    expr = parser._parse_logical_or()  # type: ignore
    if not parser._check_punct("?"):  # type: ignore
        return expr
    parser._advance()  # type: ignore
    then_expr = parser._parse_expression()  # type: ignore
    parser._expect_punct(":")  # type: ignore
    else_expr = parser._parse_conditional()  # type: ignore
...
def parse_unary(parser: object) -> Expr:
    ...
    if parser._check_keyword("sizeof"):  # type: ignore
        return parser._parse_sizeof_expr()  # type: ignore
    current = parser._current()  # type: ignore
    if current.kind == TokenKind.KEYWORD and current.lexeme in ALIGNOF_KEYWORDS:
        return parser._parse_alignof_expr()  # type: ignore
    if (
        parser._is_parenthesized_type_name_start()  # type: ignore
        and not parser._looks_like_compound_literal()  # type: ignore
    ):
        return parser._parse_cast_expr()  # type: ignore
    if parser._check_punct("++") or parser._check_punct("--"):  # type: ignore
        op = str(parser._advance().lexeme)  # type: ignore
    ...
    if any(parser._check_punct(op) for op in ("+", "-", "!", "~", "&", "*")):  # type: ignore
        op = parser._advance().lexeme  # type: ignore
        operand = parser._parse_unary()  # type: ignore
        return UnaryExpr(str(op), operand)
    return parser._parse_postfix()  # type: ignore
...
def parse_typeof_type_spec(parser: object) -> TypeSpec:
    parser._expect_punct("(")  # type: ignore
    if parser._try_parse_type_name():  # type: ignore
...
def parse_parenthesized_type_name(parser: object) -> TypeSpec:
    parser._expect_punct("(")  # type: ignore
    base_type = parser._parse_type_spec(context="type-name")  # type: ignore
    name, declarator_ops = parser._parse_declarator(  # type: ignore
        allow_abstract=True,
        allow_vla=True,
    )
    if name is not None:
    ...
    parser._expect_punct(")")  # type: ignore
    return parser._build_declarator_type(base_type, declarator_ops)  # type: ignore


def is_parenthesized_type_name_start(parser: object) -> bool:
    if not parser._check_punct("("):  # type: ignore
        return False
    index = parser._index + 1  # type: ignore
    tokens = parser._tokens  # type: ignore
    ...
    if token.kind == TokenKind.IDENT and isinstance(token.lexeme, str):
        return (
            parser._is_typedef_name(token.lexeme)  # type: ignore
...
def parse_postfix(parser: object) -> Expr:
    if parser._is_parenthesized_type_name_start():  # type: ignore
        ...
        expr = parser._parse_compound_literal_expr()  # type: ignore
    else:
        expr = parser._parse_primary()  # type: ignore
    while True:
        if parser._check_punct("("):  # type: ignore
            parser._advance()  # type: ignore
            # __builtin_va_arg(ap, type) takes a type name as 2nd argument
            if isinstance(expr, Identifier) and expr.name == "__builtin_va_arg":
                ap = parser._parse_assignment()  # type: ignore
                parser._expect_punct(",")  # type: ignore
                type_spec = parser._parse_type_name()  # type: ignore
                parser._expect_punct(")")  # type: ignore
        ...
        if parser._check_punct("["):  # type: ignore
            parser._advance()  # type: ignore
            index = parser._parse_expression()  # type: ignore
            parser._expect_punct("]")  # type: ignore
            expr = SubscriptExpr(expr, index)
            continue
        if parser._check_punct("."):  # type: ignore
            parser._advance()  # type: ignore
            member_token = parser._expect(TokenKind.IDENT)  # type: ignore
...
def looks_like_compound_literal(parser: object) -> bool:
    if not parser._is_parenthesized_type_name_start():  # type: ignore
        return False
    saved_index = parser._index  # type: ignore
    try:
        parser._parse_parenthesized_type_name()  # type: ignore
        return parser._check_punct("{")  # type: ignore
...
def parse_compound_literal_expr(parser: object) -> CompoundLiteralExpr:
    type_spec = parser._parse_parenthesized_type_name()  # type: ignore
    if not parser._check_punct("{"):  # type: ignore
        raise parser._make_error("Expected '{'", parser._current())  # type: ignore
    initializer = parser._parse_initializer_list()  # type: ignore
...
def parse_arguments(parser: object) -> list[Expr]:
    if parser._check_punct(")"):  # type: ignore
        return []
    args = [parser._parse_assignment()]  # type: ignore
    while parser._check_punct(","):  # type: ignore
        parser._advance()  # type: ignore
        args.append(parser._parse_assignment())  # type: ignore
    return args
...
def parse_primary(parser: object) -> Expr:
    ...
    if parser._check_keyword("_Generic"):  # type: ignore
        return parser._parse_generic_expr()  # type: ignore
    if token.kind == TokenKind.STRING_LITERAL:
        return parser._parse_string_literal()  # type: ignore
    if token.kind == TokenKind.IDENT and token.lexeme == "__builtin_offsetof":
        return parser._parse_builtin_offsetof()  # type: ignore
    if token.kind == TokenKind.IDENT and token.lexeme == "__builtin_types_compatible_p":
        return parser._parse_builtin_types_compatible_p()  # type: ignore
    if token.kind == TokenKind.IDENT:
        parser._advance()  # type: ignore
        assert isinstance(token.lexeme, str)
        return Identifier(token.lexeme)
    if parser._check_punct("("):  # type: ignore
        if parser._peek_punct("{"):  # type: ignore
            return parser._parse_statement_expr()  # type: ignore
        parser._advance()  # type: ignore
        expr = parser._parse_expression()  # type: ignore
        parser._expect_punct(")")  # type: ignore
        return expr
    ...
    if token.kind == TokenKind.EOF:
        raise parser._make_error("Expression is missing before end of input", token)  # type: ignore
...
def parse_builtin_offsetof(parser: object) -> BuiltinOffsetofExpr:
    ...
    token = parser._current()  # type: ignore
    if token.kind != TokenKind.IDENT:
        raise parser._make_error("Expected member name in __builtin_offsetof", token)  # type: ignore
    assert isinstance(token.lexeme, str)
...
def parse_builtin_types_compatible_p(parser: object) -> BuiltinTypesCompatExpr:
    parser._advance()  # type: ignore
    parser._expect_punct("(")  # type: ignore
    type1 = parser._parse_type_name()  # type: ignore
    parser._expect_punct(",")  # type: ignore
    type2 = parser._parse_type_name()  # type: ignore
    parser._expect_punct(")")  # type: ignore
...
def parse_generic_expr(parser: object) -> GenericExpr:
    parser._advance()  # type: ignore
    parser._expect_punct("(")  # type: ignore
    control = parser._parse_assignment()  # type: ignore
    parser._expect_punct(",")  # type: ignore
    ...
    while True:
        ...
        if parser._check_keyword("default"):  # type: ignore
            default_token = parser._current()  # type: ignore
            ...
            association_start_index = parser._index  # type: ignore
            association_type_token = parser._current()  # type: ignore
            assoc_type = replace(
                parser._parse_type_name(),  # type: ignore
            ...
            association_end_index = parser._index  # type: ignore
            association_type_spelling = parser._format_token_span(  # type: ignore
                association_start_index,
                association_end_index,
            )
            if not parser._type_name_uses_typedef_alias(  # type: ignore
                association_start_index,
                association_end_index,
            ):
                type_key = parser._generic_association_type_key(assoc_type)  # type: ignore
        ...
        parser._expect_punct(":")  # type: ignore
        associations.append((assoc_type, parser._parse_assignment()))  # type: ignore
        if not parser._check_punct(","):  # type: ignore
            break
        parser._advance()  # type: ignore
    parser._expect_punct(")")  # type: ignore
...
def parse_statement_expr(parser: object) -> StatementExpr:
    parser._expect_punct("(")  # type: ignore
    body = parser._parse_compound_stmt()  # type: ignore
    parser._expect_punct(")")  # type: ignore
    return StatementExpr(body)


def parse_string_literal(parser: object) -> StringLiteral:
    token = parser._expect(TokenKind.STRING_LITERAL)  # type: ignore
    assert isinstance(token.lexeme, str)
    prefix, body = split_string_literal(parser, token.lexeme, token)
    while parser._current().kind == TokenKind.STRING_LITERAL:  # type: ignore
        token = parser._advance()  # type: ignore
        assert isinstance(token.lexeme, str)
...
def split_string_literal(parser: object, lexeme: str, token: Token) -> tuple[str, str]:
    ...
    raise parser._make_error("Invalid string literal", token)  # type: ignore
...
def merge_string_prefix(
    ...
    if not prefix:
        return next_prefix
    raise parser._make_error("Incompatible string literal prefixes", token)  # type: ignore


def format_token_span(parser: object, start: int, end: int) -> str:
    return " ".join(
        str(token.lexeme)
        for token in parser._tokens[start:end]  # type: ignore
...
def type_name_uses_typedef_alias(parser: object, start: int, end: int) -> bool:
    for token in parser._tokens[start:end]:  # type: ignore
        if (
            ...
            and parser._is_typedef_name(token.lexeme)  # type: ignore
        ):
            return True
    return False
