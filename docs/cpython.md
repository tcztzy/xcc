...
- The current acceptance path combines curated snippet trials with compile-only checks against a pinned CPython source archive.

## Preview acceptance step

- `scripts/cpython_trial.py` covers 288 curated CPython-style snippets.
- `scripts/cpython_file_trial.py` runs compile-only frontend validation against a pinned CPython 3.11.12 tarball and a curated set of 8 real source files.
